...
router.get('/data', authMiddleware, (req, res) => {
  ...
  try { res.json({ data: JSON.parse(row.data_json) }); } catch { res.json({ data: {} }); }
});
...
router.post('/data', authMiddleware, (req, res) => {
  ...
  db.prepare('INSERT INTO user_data (email, data_json, updated_at) VALUES (?, ?, datetime(\'now\')) ON CONFLICT(email) DO UPDATE SET data_json = excluded.data_json, updated_at = datetime(\'now\')').run(req.user.email, json);
...
router.get('/sync', authMiddleware, (req, res) => {
  ...
  try { res.json({ data: JSON.parse(row.data_json), updatedAt: row.updated_at }); } catch { res.json({ data: {}, updatedAt: null }); }
});
